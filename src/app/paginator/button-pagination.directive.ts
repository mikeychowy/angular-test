...
@Directive({
  selector: '[appButtonPagination]',
  standalone: true,
})
export class ButtonPaginationDirective implements AfterViewInit, OnChanges {
  /**
   ...
   */
  private createButton(buttonText: string, i: number): HTMLElement {
    const bubbleButton = this.ren.createElement('div');
    const text = this.ren.createText(buttonText);
